...
/* ------------------------------------------------------------ */
/**
 * URI Holder. This class assists with the decoding and encoding or HTTP URI's. It differs from the java.net.URL class as it does not provide communications ability, but it does assist with query
 * string formatting.
 * <P>
...
public class URIUtil implements Cloneable {
    ...
    public static StringBuilder encodePath(StringBuilder buf, String path) {
        ...
        if (buf == null) {
            loop: for (int i = 0; i < path.length(); i++) {
                ...
                default:
                    if (c > 127) {
                        bytes = path.getBytes(DEFAULT_CHARSET);
                        buf = new StringBuilder(path.length() * 2);
                        break loop;
                    }
    ...
    public static StringBuilder encodeString(StringBuilder buf, String path, String encode) {
        ...
        synchronized (buf) {
            for (int i = 0; i < path.length(); i++) {
                ...
                if (c == '%' || encode.indexOf(c) >= 0) {
                    buf.append('%');
                    StringTools.append(buf, (byte) (0xff & c), 16);
                } else
                    buf.append(c);
            }
        }
    ...
    public static String decodePath(String path) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (b > 0) {
                String s = new String(bytes, 0, b, DEFAULT_CHARSET);
    ...
    public static String decodePath(byte[] buf, int offset, int length) {
        ...
        return new String(bytes, 0, n, DEFAULT_CHARSET);
    }
